...
import { Plus, Search, Users, BookOpen, Calendar, ChevronDown, Download } from "lucide-react";
...
import ExportBrigadeDialog from "./components/ExportBrigadeDialog";
  ...
  const filteredBrigades = brigades.filter((brigade) => {
    const matchesSearch = 
      (brigade.brigade_name?.nom?.toLowerCase() || '').includes(searchQuery.toLowerCase())
       ...
       ||   (brigade.effectif?.toString().toLowerCase() || '').includes(searchQuery.toLowerCase());
      ...
      <div className="bg-card border border-border rounded-lg p-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Rechercher par stage, spécialité, effectif..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="placeholder:text-sm w-full pl-10 pr-4 py-2 bg-background border border-input rounded-lg focus:outline-none focus:ring-2 focus:ring-ring focus:border-transparent text-foreground "
            />
          </div>
